<?php
/**
 * Class for the import actions used in the Crafthemes Demo Import plugin.
 * Register default WP actions for CT_CTDI plugin.
 *
 * @package CT_CTDI
 */
...
class ImportActions {
	...
	public function register_hooks() {
		...
		add_action( 'ct-CT_CTDI/before_content_import_execution', array( $this, 'before_content_import_action' ), 10, 3 );

		// After content import.
		add_action( 'ct-CT_CTDI/after_content_import_execution', array( $this, 'before_widget_import_action' ), 10, 3 );
		add_action( 'ct-CT_CTDI/after_content_import_execution', array( $this, 'widgets_import' ), 20, 3 );
		add_action( 'ct-CT_CTDI/after_content_import_execution', array( $this, 'redux_import' ), 30, 3 );

		// Customizer import.
		add_action( 'ct-CT_CTDI/customizer_import_execution', array( $this, 'customizer_import' ), 10, 1 );

		// After full import action.
		add_action( 'ct-CT_CTDI/after_all_import_execution', array( $this, 'after_import_action' ), 10, 3 );

		// Special widget import cases.
		if ( apply_filters( 'pt_CT_CTDI/enable_custom_menu_widget_ids_fix', true ) ) {
			add_action( 'ct-CT_CTDI/widget_settings_array', array( $this, 'fix_custom_menu_widget_ids' ) );
		}
	}
	...
	public function fix_custom_menu_widget_ids( $widget ) {
		...
		$content_import_data = $CT_CTDI->importer->get_importer_data();
	 ...
	 * @param array $import_files          The filtered import files defined in `ct-CT_CTDI/import_files` filter.
	...
	/**
	 * Execute the action: 'ct-CT_CTDI/before_content_import'.
	 *
	 * @param array $selected_import_files Actual selected import files (content, widgets, customizer, redux).
	 * @param array $import_files          The filtered import files defined in `ct-CT_CTDI/import_files` filter.
	 * @param int   $selected_index        Selected index of import.
	...
	public function before_content_import_action( $selected_import_files, $import_files, $selected_index ) {
		$this->do_import_action( 'ct-CT_CTDI/before_content_import', $import_files[ $selected_index ] );
	}


	/**
	 * Execute the action: 'ct-CT_CTDI/before_widgets_import'.
	 *
	 * @param array $selected_import_files Actual selected import files (content, widgets, customizer, redux).
	 * @param array $import_files          The filtered import files defined in `ct-CT_CTDI/import_files` filter.
	 * @param int   $selected_index        Selected index of import.
	...
	public function before_widget_import_action( $selected_import_files, $import_files, $selected_index ) {
		$this->do_import_action( 'ct-CT_CTDI/before_widgets_import', $import_files[ $selected_index ] );
	}


	/**
	 * Execute the action: 'ct-CT_CTDI/after_import'.
	 *
	 * @param array $selected_import_files Actual selected import files (content, widgets, customizer, redux).
	 * @param array $import_files          The filtered import files defined in `ct-CT_CTDI/import_files` filter.
	 * @param int   $selected_index        Selected index of import.
	...
	public function after_import_action( $selected_import_files, $import_files, $selected_index ) {
		$this->do_import_action( 'ct-CT_CTDI/after_import', $import_files[ $selected_index ] );
	}
	 ...
	 * @param array  $selected_import The data of selected import from `ct-CT_CTDI/import_files` filter.
	 */
