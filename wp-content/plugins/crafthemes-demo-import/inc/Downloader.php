...
class Downloader {
	...
	private function get_content_from_url( $url ) {
		...
		if ( empty( $url ) ) {
			return new \WP_Error(
				'missing_url',
				__( 'Missing URL for downloading a file!', 'ct-ctdi' )
			);
		}
		...
		$response = wp_remote_get(
			$url,
			array( 'timeout' => apply_filters( 'ct-CT_CTDI/timeout_for_downloading_import_file', 20 ) )
		);
		...
		if ( is_wp_error( $response ) || 200 !== $response['response']['code'] ) {
			...
			return new \WP_Error(
				'download_error',
				sprintf(
					__( 'An error occurred while fetching file from: %1$s%2$s%3$s!%4$sReason: %5$s - %6$s.', 'ct-ctdi' ),
					'<strong>',
					$url,
					'</strong>',
					'<br>',
				...
				apply_filters( 'ct-CT_CTDI/message_after_file_fetching_error', '' )
			);
		}
	...
	public function set_download_directory_path( $download_directory_path ) {
		if ( file_exists( $download_directory_path ) ) {
			...
			$this->download_directory_path = apply_filters( 'ct-CT_CTDI/upload_file_path', trailingslashit( $upload_dir['path'] ) );
		}
	}
}
