<?php
/**
 * Class for declaring the content importer used in the Crafthemes Demo Import plugin
 *
 * @package CT_CTDI
 */
...
class Importer {
	...
	/**
	 * The instance of the CT_CTDI\Logger class.
	 *
	 * @var object
	 */
	public $logger;
	...
	/**
	 * The instance of the Crafthemes Demo Import class.
	 *
	 * @var object
	 */
	private $CT_CTDI;
	...
	public function __construct( $importer_options = array(), $logger = null ) {
		...
		// Get the CT_CTDI (main plugin class) instance.
	...
	public function import_content( $import_file_path ) {
		...
		set_time_limit( apply_filters( 'ct-CT_CTDI/set_time_limit_for_demo_data_import', 300 ) );
		...
		if ( ! apply_filters( 'ct-CT_CTDI/regenerate_thumbnails_in_content_import', true ) ) {
	...
	public function new_ajax_request_maybe( $data ) {
		...
		if ( $time > apply_filters( 'ct-CT_CTDI/time_for_one_ajax_call', 25 ) ) {
			$response = array(
				'status'  => 'newAJAX',
			...
			if ( ! empty( $message ) ) {
				$this->CT_CTDI->append_to_frontend_error_messages( $message );
			}

			// Add message to log file.
			$log_added = Helpers::append_to_file(
				__( 'New AJAX call!' , 'ct-ctdi' ) . PHP_EOL . $message,
				$this->CT_CTDI->get_log_file_path(),
				''
			);
	...
	private function set_current_importer_data() {
		$data = array_merge( $this->CT_CTDI->get_current_importer_data(), $this->get_importer_data() );

		Helpers::set_CT_CTDI_import_data_transient( $data );
	}
}
