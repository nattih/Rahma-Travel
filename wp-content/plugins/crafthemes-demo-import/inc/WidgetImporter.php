<?php
/**
 * Class for the widget importer used in the Crafthemes Demo Import plugin.
 *
...
class WidgetImporter {
	...
	public static function import( $widget_import_file_path ) {
		$results       = array();
		$CT_CTDI          = CrafthemesClickDemoImport::get_instance();
		$log_file_path = $CT_CTDI->get_log_file_path();
		...
		if ( is_wp_error( $results ) ) {
			...
			// Add any error messages to the frontend_error_messages variable in CT_CTDI main class.
			$CT_CTDI->append_to_frontend_error_messages( $error_message );

			// Write error to log file.
			Helpers::append_to_file(
				$error_message,
				$log_file_path,
				esc_html__( 'Importing widgets', 'ct-ctdi' )
			);
		}
		else {
			ob_start();
	...
	private static function process_import_file( $file ) {
		...
		if ( ! file_exists( $file ) ) {
			return new \WP_Error(
				...
				__( 'Error: Widget import file could not be found.', 'ct-ctdi' )
			);
		}
	...
	private static function import_data( $data ) {
		...
		if ( empty( $data ) || ! is_object( $data ) ) {
			return new \WP_Error(
				...
				__( 'Error: Widget import data could not be read. Please try a different file.', 'ct-ctdi' )
			);
		}

		// Hook before import.
		do_action( 'ct-CT_CTDI/widget_importer_before_widgets_import' );
		$data = apply_filters( 'ct-CT_CTDI/before_widgets_import_data', $data );
		...
		foreach ( $data as $sidebar_id => $widgets ) {
			...
			if ( isset( $wp_registered_sidebars[ $sidebar_id ] ) ) {
				...
				$sidebar_message      = __( 'Sidebar does not exist in theme (moving widget to Inactive)', 'ct-ctdi' );
			}
			...
			foreach ( $widgets as $widget_instance_id => $widget ) {
				...
				if ( ! $fail && ! isset( $available_widgets[ $id_base ] ) ) {
					...
					$widget_message      = __( 'Site does not support widget', 'ct-ctdi' ); // Explain why widget not imported.
				}

				// Filter to modify settings object before conversion to array and import.
				// Leave this filter here for backwards compatibility with manipulating objects (before conversion to array below).
				// Ideally the newer wie_widget_settings_array below will be used instead of this.
				$widget = apply_filters( 'ct-CT_CTDI/widget_settings', $widget ); // Object.
				...
				$widget = apply_filters( 'ct-CT_CTDI/widget_settings_array', $widget );
				...
				if ( ! $fail && isset( $widget_instances[ $id_base ] ) ) {
					...
					foreach ( $single_widget_instances as $check_id => $check_widget ) {
						...
						if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
							...
							$widget_message      = __( 'Widget already exists', 'ct-ctdi' ); // Explain why widget not imported.
				...
				if ( ! $fail ) {
					...
					do_action( 'ct-CT_CTDI/widget_importer_after_single_widget_import', $after_widget_import );

					// Success message.
					if ( $sidebar_available ) {
						$widget_message_type = 'success';
						$widget_message      = __( 'Imported', 'ct-ctdi' );
					}
					else {
						$widget_message_type = 'warning';
						$widget_message      = __( 'Imported to Inactive', 'ct-ctdi' );
					}
				}

				// Result for widget instance.
				$results[ $sidebar_id ]['widgets'][ $widget_instance_id ]['name']         = isset( $available_widgets[ $id_base ]['name'] ) ? $available_widgets[ $id_base ]['name'] : $id_base; // Widget name or ID if name not available (not supported by site).
				$results[ $sidebar_id ]['widgets'][ $widget_instance_id ]['title']        = ! empty( $widget['title'] ) ? $widget['title'] : __( 'No Title', 'ct-ctdi' ); // Show "No Title" if widget instance is untitled.
		...
		// Hook after import.
		do_action( 'ct-CT_CTDI/widget_importer_after_widgets_import' );

		// Return results.
		return apply_filters( 'ct-CT_CTDI/widget_import_results', $results );
	}
	...
	private static function available_widgets() {
		...
		return apply_filters( 'ct-CT_CTDI/available_widgets', $available_widgets );
	}
	...
	private static function format_results_for_log( $results ) {
		if ( empty( $results ) ) {
			esc_html_e( 'No results for widget import!', 'ct-ctdi' );
		}
