<?php
/**
 * Class for the customizer importer used in the Crafthemes Demo Import plugin.
 *
	...
	public static function import( $customizer_import_file_path ) {
		...
		if ( is_wp_error( $results ) ) {
			...
			Helpers::append_to_file(
				$error_message,
				$log_file_path,
				esc_html__( 'Importing customizer settings', 'ct-ctdi' )
			);
		}
		else {
			// Add this message to log file.
			$log_added = Helpers::append_to_file(
				esc_html__( 'Customizer settings import finished!', 'ct-ctdi' ),
				$log_file_path,
				esc_html__( 'Importing customizer settings' , 'ct-ctdi' )
			);
		}
	...
	public static function import_customizer_options( $import_file_path ) {
		...
		if ( ! file_exists( $import_file_path ) ) {
			return new \WP_Error(
				...
				sprintf(
					esc_html__( 'Error: The customizer import file is missing! File path: %s', 'ct-ctdi' ),
					$import_file_path
				)
			);
		}
		...
		if ( ! is_array( $data ) && ( ! isset( $data['template'] ) || ! isset( $data['mods'] ) ) ) {
			return new \WP_Error(
				...
				esc_html__( 'Error: The customizer import file is not in a correct format. Please make sure to use the correct customizer import file.', 'ct-ctdi' )
			);
		}
		if ( $data['template'] !== $template ) {
			return new \WP_Error(
				'customizer_import_wrong_theme',
				esc_html__( 'Error: The customizer import file is not suitable for current theme. You can only import customizer settings for the same theme or a child theme.', 'ct-ctdi' )
			);
		}

		// Import images.
		if ( apply_filters( 'ct-CT_CTDI/customizer_import_images', true ) ) {
		...
		$use_wp_customize_save_hooks = apply_filters( 'ct-CT_CTDI/enable_wp_customize_save_hooks', false );
