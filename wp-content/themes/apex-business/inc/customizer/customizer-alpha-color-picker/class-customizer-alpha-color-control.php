...
define( 'APEX_BUSINESS_ALPHA_VERSION', '1.0.0' );

/**
 * Class Hestia_Customize_Alpha_Color_Control
 */
class Apex_Business_Customizer_Alpha_Color_Control extends WP_Customize_Control {
	/**
	...
	public function enqueue() {
		wp_enqueue_script(
			...
			get_template_directory_uri() . '/inc/customizer/customizer-alpha-color-picker/js/alpha-color-picker.js',
			array( 'jquery', 'wp-color-picker' ),
			APEX_BUSINESS_ALPHA_VERSION,
			true
		);
		wp_enqueue_style(
			'customizer-alpha-color-picker',
			get_template_directory_uri() . '/inc/customizer/customizer-alpha-color-picker/css/alpha-color-picker.css',
	...
	public function render_content() {
		...
		<label>
			...
			if ( isset( $this->label ) && '' !== $this->label ) {
				echo '<span class="customize-control-title ct-alpha" val="' . esc_attr( $this->label ) . '">' . esc_html( $this->label ) . '</span>';
			}
