...
define( 'APEX_BUSINESS_RANGE_VERSION', '1.0.0' );
 ...
 * @access public
 */
class Apex_Business_Customizer_Range_Value_Control extends WP_Customize_Control {
	...
	public function enqueue() {
		wp_enqueue_script( 'customizer-range-value-control', get_template_directory_uri() . '/inc/customizer/customizer-range-control/js/customizer-range-value-control.js', array( 'jquery', 'customize-base' ), APEX_BUSINESS_RANGE_VERSION, true );
		wp_enqueue_style( 'customizer-range-value-control', get_template_directory_uri() . '/inc/customizer/customizer-range-control/css/customizer-range-value-control.css', array(), APEX_BUSINESS_RANGE_VERSION );
	}
 ...
 */
function apex_business_sanitize_range_value( $input ) {
	if ( apex_business_is_json( $input ) ) {
 ...
 * @param  string $string Input.
 */
function apex_business_is_json( $string ) {
	return is_string( $string ) && is_array( json_decode( $string, true ) ) ? true : false;
}
