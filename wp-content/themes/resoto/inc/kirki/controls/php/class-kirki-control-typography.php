		...
		<div class="wrapper">
			...
			<# if ( ! _.isUndefined( data.default['font-family'] ) ) { #>
				...
				<div class="font-family">
					<h5><?php esc_html_e( 'Font Family', 'resoto' ); ?></h5>
					<select {{{ data.inputAttrs }}} id="kirki-typography-font-family-{{{ data.id }}}" placeholder="<?php esc_attr_e( 'Select Font Family', 'resoto' ); ?>"></select>
				</div>
				<# if ( ! _.isUndefined( data.choices['font-backup'] ) && true === data.choices['font-backup'] ) { #>
					<div class="font-backup hide-on-standard-fonts kirki-font-backup-wrapper">
						<h5><?php esc_html_e( 'Backup Font', 'resoto' ); ?></h5>
						<select {{{ data.inputAttrs }}} id="kirki-typography-font-backup-{{{ data.id }}}" placeholder="<?php esc_attr_e( 'Select Font Family', 'resoto' ); ?>"></select>
					</div>
				<# } #>
				<# if ( true === data.show_variants || false !== data.default.variant ) { #>
					<div class="variant kirki-variant-wrapper">
						<h5><?php esc_html_e( 'Variant', 'resoto' ); ?></h5>
			...
			<# if ( ! _.isUndefined( data.default['font-size'] ) ) { #>
				...
				<div class="font-size">
					<h5><?php esc_html_e( 'Font Size', 'resoto' ); ?></h5>
			...
			<# if ( ! _.isUndefined( data.default['line-height'] ) ) { #>
				...
				<div class="line-height">
					<h5><?php esc_html_e( 'Line Height', 'resoto' ); ?></h5>
			...
			<# if ( ! _.isUndefined( data.default['letter-spacing'] ) ) { #>
				...
				<div class="letter-spacing">
					<h5><?php esc_html_e( 'Letter Spacing', 'resoto' ); ?></h5>
			...
			<# if ( ! _.isUndefined( data.default['word-spacing'] ) ) { #>
				...
				<div class="word-spacing">
					<h5><?php esc_html_e( 'Word Spacing', 'resoto' ); ?></h5>
			...
			<# if ( ! _.isUndefined( data.default['text-align'] ) ) { #>
				...
				<div class="text-align">
					<h5><?php esc_html_e( 'Text Align', 'resoto' ); ?></h5>
					<div class="text-align-choices">
						<input {{{ data.inputAttrs }}} type="radio" value="inherit" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-inherit" <# if ( data.value['text-align'] === 'inherit' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-inherit">
								...
								<span class="screen-reader-text"><?php esc_html_e( 'Inherit', 'resoto' ); ?></span>
							</label>
						</input>
						<input {{{ data.inputAttrs }}} type="radio" value="left" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-left" <# if ( data.value['text-align'] === 'left' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-left">
								...
								<span class="screen-reader-text"><?php esc_html_e( 'Left', 'resoto' ); ?></span>
							</label>
						</input>
						<input {{{ data.inputAttrs }}} type="radio" value="center" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-center" <# if ( data.value['text-align'] === 'center' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-center">
								...
								<span class="screen-reader-text"><?php esc_html_e( 'Center', 'resoto' ); ?></span>
							</label>
						</input>
						<input {{{ data.inputAttrs }}} type="radio" value="right" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-right" <# if ( data.value['text-align'] === 'right' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-right">
								...
								<span class="screen-reader-text"><?php esc_html_e( 'Right', 'resoto' ); ?></span>
							</label>
						</input>
						<input {{{ data.inputAttrs }}} type="radio" value="justify" name="_customize-typography-text-align-radio-{{ data.id }}" id="{{ data.id }}-text-align-justify" <# if ( data.value['text-align'] === 'justify' ) { #> checked="checked"<# } #>>
							<label for="{{ data.id }}-text-align-justify">
								...
								<span class="screen-reader-text"><?php esc_html_e( 'Justify', 'resoto' ); ?></span>
							</label>
						</input>
					</div>
				</div>
			...
			<# if ( ! _.isUndefined( data.default['text-transform'] ) ) { #>
				...
				<div class="text-transform">
					<h5><?php esc_html_e( 'Text Transform', 'resoto' ); ?></h5>
					<select {{{ data.inputAttrs }}} id="kirki-typography-text-transform-{{{ data.id }}}">
						...
						<option value="none"<# if ( 'none' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_html_e( 'None', 'resoto' ); ?></option>
						<option value="capitalize"<# if ( 'capitalize' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_html_e( 'Capitalize', 'resoto' ); ?></option>
						<option value="uppercase"<# if ( 'uppercase' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_html_e( 'Uppercase', 'resoto' ); ?></option>
						<option value="lowercase"<# if ( 'lowercase' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_html_e( 'Lowercase', 'resoto' ); ?></option>
						<option value="initial"<# if ( 'initial' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_html_e( 'Initial', 'resoto' ); ?></option>
						<option value="inherit"<# if ( 'inherit' === data.value['text-transform'] ) { #>selected<# } #>><?php esc_html_e( 'Inherit', 'resoto' ); ?></option>
					</select>
				</div>
			<# } #>
			...
			<# if ( ! _.isUndefined( data.default['text-decoration'] ) ) { #>
				...
				<div class="text-decoration">
					<h5><?php esc_html_e( 'Text Decoration', 'resoto' ); ?></h5>
					<select {{{ data.inputAttrs }}} id="kirki-typography-text-decoration-{{{ data.id }}}">
						...
						<option value="none"<# if ( 'none' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_html_e( 'None', 'resoto' ); ?></option>
						<option value="underline"<# if ( 'underline' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_html_e( 'Underline', 'resoto' ); ?></option>
						<option value="overline"<# if ( 'overline' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_html_e( 'Overline', 'resoto' ); ?></option>
						<option value="line-through"<# if ( 'line-through' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_html_e( 'Line-Through', 'resoto' ); ?></option>
						<option value="initial"<# if ( 'initial' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_html_e( 'Initial', 'resoto' ); ?></option>
						<option value="inherit"<# if ( 'inherit' === data.value['text-decoration'] ) { #>selected<# } #>><?php esc_html_e( 'Inherit', 'resoto' ); ?></option>
					</select>
				</div>
			<# } #>
			...
			<# if ( ! _.isUndefined( data.default['margin-top'] ) ) { #>
				...
				<div class="margin-top">
					<h5><?php esc_html_e( 'Margin Top', 'resoto' ); ?></h5>
			...
			<# if ( ! _.isUndefined( data.default['margin-bottom'] ) ) { #>
				...
				<div class="margin-bottom">
					<h5><?php esc_html_e( 'Margin Bottom', 'resoto' ); ?></h5>
			...
			<# if ( ! _.isUndefined( data.default['color'] ) && false !== data.default['color'] ) { #>
				...
				<div class="color">
					<h5><?php esc_html_e( 'Color', 'resoto' ); ?></h5>
